...
### _Affine2By3_ (variable 2×3 matrix)
...
### _PaintFormat4 Table_ (group)

|Type | Field name | Description |
|-|-|-|
| uint16 | format | set to 4 |
| VarF2Dot14 | alpha | alpha.value, and all variations of it, must be in the range 0 to 1 (inclusive) |
| Offset32 | 2By3TransformOffset | Offset to Affine2By3 table, from start of PaintFormat4 table. May be NULL. |

(Size (header, excluding Affine2By3 subtable): 2 + 6 + 4 12 bytes)

A LayerList table will include a LayerV1Record that joins the PaintFormat4 with a glyph ID. If that GID corresponds to a BaseGlyphV1Record in the BaseGlyphV1List table, then the layers of that glyph are taken as a group, with the transform and alpha indicated in the PaintFormat4 fields. Otherwise, the glyph ID is taken as a solid color layer, similar to PaintFormat1 but using the text foreground color.
